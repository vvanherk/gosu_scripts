...
function generateQuoteSheet(quote : QuoteLoggerSearchResult) {
  ...
  var targetExcelLog = new java.io.File(LOG_DIR, "${quote.CreateTime.format(OUTPUT_TF)}.${ctx.QUOTE_SHEET_REFID}.xlsm.log")
  var os = new FileOutputStream(targetExcelLog)
  wb.write(os)

  if (SAVE_XML) {
    var RB_REQ_XML = new java.io.File(LOG_DIR, "${ctx.QUOTE_SHEET_REFID}.RB_REQUEST.xml.log")
    var RB_RES_XML = new java.io.File(LOG_DIR, "${ctx.QUOTE_SHEET_REFID}.RB_RESPONSE.xml.log")
